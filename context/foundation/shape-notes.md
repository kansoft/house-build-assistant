...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain framing"
      decision: "Decision paralysis at stage transitions, missing pre-stage preparation, and cross-trade coordination are facets of the same problem — checklist + readiness assessment serves all three."
    - topic: "primary persona"
      decision: "First-time individual investor building their own house (non-professional, learning as they go)."
    - topic: "auth strategy"
      decision: "Email + password login. Server-stored per-user data."
    - topic: "role model"
      decision: "Flat — each user owns their own investments. No sharing, no admin, no contractor view in MVP."
    - topic: "MVP scope vs timeline"
      decision: "Commit to full scope (auth + 10–12 stage seeded taxonomy + CRUD + readiness rule + AI generation). 6-week MVP, sustained-effort cost accepted."
    - topic: "stage taxonomy granularity"
      decision: "10–12 fine-grained stages (e.g. 'instalacje przed tynkami' and 'instalacje po tynkach' separate; 'stan surowy otwarty' and 'stan surowy zamknięty' separate). Closer to how investors actually phase work."
    - topic: "checklist item editability"
      decision: "Default (seeded) items are read-only and can only be checked/unchecked. Custom items added by the user are fully editable and deletable. AI-suggested items, once accepted, are treated as custom."
    - topic: "AI role in checklist"
      decision: "AI augments the seeded default by suggesting additional items. Investor accepts or dismisses each suggestion individually; accepted items become custom items."
    - topic: "readiness verdict timing"
      decision: "Always-visible live indicator (ready / partially ready / has risks) updates after every toggle. Detailed report (missing items, risk items, next-step recommendation) opens on explicit click."
    - topic: "readiness rule shape"
      decision: "Each checklist item carries a criticality flag (critical | standard). A stage is 'ready to close' when all critical items are checked or marked N/A; 'has risks' when at least one critical item remains unchecked; 'partially ready' when all critical items are handled but standard items remain unchecked."
    - topic: "cross-stage dependencies"
      decision: "MVP evaluates each stage standalone. Cross-stage hints ('before tynki verify all instalacje items') are out of scope for v1."
    - topic: "extra NFRs beyond guardrails"
      decision: "Bounded AI suggestion latency (with explicit timeout messaging) + Polish-language UX (seeded content, UI labels, AI prompts all in Polish)."
  frs_drafted: 22
  quality_check_status: accepted
---

# HouseBuild Assistant — Shape Notes

Seed input: `housebuild-assistant-mvp.md` (provided by user).
...
The insight: stage-gated, residential-construction-specific checklists do not exist as a product. Generic project trackers (Trello, Notion) require the investor to author the checklist themselves — which assumes they already know what to check, which is the exact knowledge they lack. Forum threads and contractor advice are unstructured and partial. A purpose-built checklist that knows construction stages and the cross-trade dependencies between them, paired with a structured readiness assessment, is the missing tool.
...
Acknowledged on 2026-05-25: 6-week MVP requires sustained dedication over multiple evenings/weekends; user accepted the sustained-effort cost in Step 3. Full scope (auth + 6-stage seeded checklists + CRUD + readiness rule + AI generation + E2E test + CI/CD) is committed for v1.
...
- FR-020: Investor can open a detailed readiness report listing missing items, risk items, and a recommended next step. Priority: must-have
  > Socrates: Counter-argument considered: "always-visible inline so the report isn't hidden behind a click." Resolution: stands as written; the click is the deliberate moment of evaluation. UI placement (inline-collapsed vs modal) is a downstream design choice.
...
- Toggling an item visibly updates the readiness indicator without a full page reload
...
- The readiness indicator reflects every item toggle within continuous-feedback latency (no full-page reload, no perceptible lag between toggling an item and seeing the verdict update).
- User-facing content — stage names, seeded checklist items, UI labels, AI prompts, and AI-suggested item text — is in Polish. The product is Polish-only for v1; later locale addition does not require re-authoring seeded content (no Polish strings are embedded inside the readiness logic itself).
...
- **Native mobile app** — web-only for v1 (responsive UI is fine, native is v2+).
...
- **Cross-stage dependency hints** — v1 evaluates each stage standalone (per Phase 5 decision). Hints like "before tynki, verify all instalacje items" are deferred to v2.
...
- **Offline-first usage** — web app requires network; no service worker, no offline-capable behavior in v1.
- **Multi-language at launch** — Polish-only for v1. Structure permits later locale addition but no other locales are shipped.

## Forward: tech-stack (not part of PRD — handoff for the next chain step)

The seed doc (`housebuild-assistant-mvp.md`, "Proponowany stack") proposes the following technologies. Captured here verbatim for the downstream `10x-tech-stack-selector` step. These are NOT PRD content and do NOT bind the product shape.

- Frontend: React 19 + Astro 6 + TypeScript
- UI: Mantine or shadcn/ui (not decided)
...
The downstream tech-stack-selector step should evaluate this proposal against the product priors captured in PRD frontmatter (web-app, medium scale, 6-week after-hours timeline) and the NFRs (AI latency, Polish-language UX, per-user isolation, data durability). The investor's seed proposal carries weight but is not binding.

## Forward: technical-roadmap (not part of PRD)

The seed doc captures success criteria that include "Projekt posiada pipeline CI/CD uruchamiający build i testy" and "Co najmniej jeden główny przepływ użytkownika jest pokryty testem E2E". These are testing and deployment concerns that belong to a downstream technical-roadmap step, not to PRD. Captured here so the chain step picks them up:

- Pipeline CI/CD running build + tests (proposed: GitHub Actions per seed doc).
- At least one E2E test covering the primary user flow (proposed: Playwright per seed doc, scenario described in `## Proponowany test E2E` of the seed).
- E2E scenario: sign-in → create investment → select "Instalacje przed tynkami" → mark items → see readiness assessment → after filling required items, see "ready to close" verdict.

## Quality cross-check

All five greenfield quality gates passed on 2026-05-25:

- **Access Control** — present (email + password, flat per-user role model, password reset required).
- **Business Logic** — present, one-sentence rule captured ("The app classifies each construction stage's readiness — ready to close, partially ready, or has risks — by checking which of the stage's critical checklist items remain unchecked, and surfaces the missing items by name so the investor can decide whether to release the trade.").
- **Project artifacts** — `shape-notes.md` present with full checkpoint.
- **Timeline-cost acknowledged** — 6-week MVP explicitly accepted in Phase 3 (see `## Timeline acknowledgment` block).
- **Non-Goals** — 13 functional + 2 non-functional non-goals captured.

Status: `accepted`. No warnings to mirror into PRD's `## Open Questions`.

## Open Questions (for PRD)

Items surfaced during shaping that need downstream resolution; `/10x-prd` should mirror these verbatim into the PRD's `## Open Questions` section.

1. **Who authors the seeded checklist content for the 10–12 construction stages?** — Owner: user. Required before v1 launch. The taxonomy IS the product value (per FR-008 Socrates resolution); empty or low-quality seeded content makes the product hollow regardless of features built.
2. **What is the criticality flag distribution per seeded item?** — Owner: user, co-authored with the checklist content. Each seeded item needs `critical | standard` set when authored; the readiness rule depends on this.
3. **Bounded AI suggestion latency — what is the user-perceived timeout threshold?** — Owner: design / downstream. NFR captures "bounded" without naming a number; downstream design picks the value (typical web-AI patterns are 5–15s).
4. **Should AI suggestions be sourced or cited?** — Surfaced during FR-016 Socrates challenge. Resolution: deferred — v1 ships suggestions without source attribution, but the product may need this if quality complaints surface.
5. **Should AI suggestions default to opt-in or always-available?** — Surfaced during FR-016 Socrates challenge. Resolution: deferred — v1 makes AI invokable on demand; default state is a downstream UX choice.
6. **Custom-stage UX (FR-021): how does the investor describe their stage so the readiness rule still makes sense?** — Owner: design. Custom stages start empty; the investor has to add items themselves or rely on AI generation. Open: does the investor name the stage in plain text, or pick a "based on" parent from the taxonomy?
7. **N/A marker UX (FR-022): how is "not applicable" visually distinct from "not checked"?** — Owner: design. Critical that the rule output is unambiguous.
8. **Polish-only at launch — when (if ever) does English / other locales land?** — Owner: user / product roadmap.
