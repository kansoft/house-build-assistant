...
HouseBuild Assistant — Astro 6 (`output: "server"`) + React 19 + Tailwind 4, auth/data via Supabase (SSR), deployed to Cloudflare Workers. Single app, npm, Node 22.14.0 (`.nvmrc`). Product context: @context/foundation/prd.md and @context/foundation/tech-stack.md.

## Commands

- `npm run dev` — local dev server. `npm run build` runs `astro build`; run `npx astro sync` first if `astro:*` types are stale.
- `npm run lint` / `npm run lint:fix`, `npm run format` (Prettier). No test runner is set up — do not assume `npm test` exists.
- Pre-commit runs husky + lint-staged (ESLint on `*.{ts,tsx,astro}`, Prettier on `*.{json,css,md}`), so commits can fail on lint.

## Conventions & gotchas

- Import alias `@/` → `src/` (e.g. `@/lib/supabase`).
